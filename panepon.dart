...
// 座標を指定してパネルを移動させる
// 座標は左側のマスを指定すること
String manualChange(String coordinate, panel, abc) {
...
void order(int operation) {
  ...
  for (int i = 0; i < 55; i++) {
    ...
    String checkSkip = manualChange(first[i], initPanels, abcdef);
    if (checkSkip == 'skip') {
      continue;
    }
    sequence(initPanels);
    ...
    for (int ii = 0; ii < 55; ii++) {
      ...
      sequence(initPanels);
      String checkSkip = manualChange(second[ii], initPanels, abcdef);
      if (checkSkip == 'skip') {
        continue;
      }
      sequence(initPanels);
      ...
      for (int iii = 0; iii < 55; iii++) {
        ...
        sequence(initPanels);
        String checkSkip = manualChange(third[iii], initPanels, abcdef);
        if (checkSkip == 'skip') {
          continue;
        }
        sequence(initPanels);
        ...
        for (int iiii = 0; iiii < 55; iiii++) {
          ...
          sequence(initPanels);
          String checkSkip = manualChange(forth[iiii], initPanels, abcdef);
          if (checkSkip == 'skip') {
            continue;
          }
          sequence(initPanels);
